...
SYSTEM_PROMPT = (
    ...
    "- A PDF may also already be ingested from outside this conversation (e.g. "
    "uploaded via the app's sidebar). So whenever the user asks about "
    "'the document', 'the PDF', 'the file', or any question that isn't "
    "obviously general knowledge, call `search_pdf` first to check before "
    "answering or before asking the user for a path/URL — only ask for one if "
    "`search_pdf` reports nothing has been ingested yet.\n"
